...
def trigger_scrape(request):
    ...
    try:
        ...
        while True:  # Keep looping through all pages until no next button
                ...
                time.sleep(10)
                monologue = driver.find_elements(By.CSS_SELECTOR, 'td.mat-cell>span.link')
                time.sleep(5)
                print(monologue)
                print('fsjdkfsadjkfskldfksaldfksdfklaslsdafklsalkskdl')
                for i in range(len(monologue)):
                    ...
                    print(data_elements_2)
                    time.sleep(5)
                    _create_status(new_run, "Fetching inside infinite loop",) 
